...
/// system will panic.
pub async fn initialise_io(red: bool, amber: bool, green: bool, led4: bool) {
...
/// Control the on-board led, named `LED4` in the schematic and the silkscreen
/// on the PCB. Leds `LED1` (power), `LED2` (serial RX) and `LED3` (serial TX)
/// cannot be controlled from code. They have been hardwired.
///
/// `LED4` is active-low, but this function handles that. Pass in `true` to turn
/// the led on and `false` to switch it off.
pub async fn light_led4(on: bool) {
    let mut led_guard: MutexGuard<'_, ThreadModeRawMutex, Option<Output>> = LED4.lock().await;
    let led4: &mut Output = led_guard.as_mut().expect(IO_INIT_PANIC);

    // the onboard LED4 is active-low
    led4.set_level(if on { Level::Low } else { Level::High });
}
